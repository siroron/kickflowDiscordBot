...
- **アクセストークン**: ユーザー自身で発行可能（kickflow のユーザー設定 →「アクセストークン設定」）。
  本システムはこのトークンを `KICKFLOW_ACCESS_TOKEN` として使う。
- **Webhook（kickflow → 外部の通知）**: 現在ユーザー権限では設定不可。
  そのため、承認結果を Discord にプッシュ通知する拡張機能は本リポジトリの対象外とする。
  必要になった場合は kickflow 管理者に Webhook 設定を依頼する必要がある。

サービスアカウント（bot が複数ユーザーになりすまして申請）も管理者権限が必要だが、
最小構成では「個人のアクセストークンで作成 → 申請者は本人」で十分。
...
- [ ] アクセストークンの発行（ユーザー設定 →「アクセストークン設定」から自身で発行）
...
Webhook（kickflow → 外部）はユーザー権限で設定できないため、本リポジトリの対象外。
